...
func MakeApplicationCreateTx(
	...
	tx, err := future.MakeApplicationCreateTx(optIn, approvalProg, clearProg, globalSchema, localSchema, appArgs.Extract(), accounts.Extract(), internalForeignApps, internalForeignAssets, internalParams, senderAddr, note, types.Digest{}, [32]byte{}, types.Address{}, uint32(extraPages))
	if err == nil {
		encoded = msgpack.Encode(tx)
	}
